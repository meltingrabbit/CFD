...
void CalcCoefficient(int tStep);
	...
	REP(tStep,T_STEP_END+1) {
		time += dt;
			...
			if (tStep%PRINT_CONSOLE_MOD == 0) {
				ps(tStep, "\t", GetFNotation(time,3,1), "\t");
			}
		#endif
...
void SolveVelocityEquation(int tStep) {
	...
	for (int i=0+2;i<=NX-2;++i) {
		for (int j=0+2;j<=NY-2;++j) {
			...
			// rhsu = 0;
			// rhsv = 0;
			// if ((i1-1 <= i && i <= i2+1) && (j1-1 <= j && j <= j2+1)) continue;
			// rhsu = -U[i][j] * (-U[i+2][j] + 8.0 * (U[i+1][j] - U[i-1][j]) + U[i-2][j]) / 12.0 / dx;
			// rhsu += -abs(U[i][j]) * (U[i+2][j] - 4.0 * U[i+1][j] + 6.0 * U[i][j] - 4.0 * U[i-1][j] + U[i-2][j]) / 4.0 / dx;
			rhsu += -(P[i+1][j] - P[i-1][j]) * 0.5 * invDx;
			rhsv += -(P[i][j+1] - P[i][j-1]) * 0.5 * invDy;
			// rhsu = -(P[i+1][j] - P[i-1][j]) / 2.0 / dx;
			// rhsv = -(P[i][j+1] - P[i][j-1]) / 2.0 / dy;
			rhsu += (U[i+1][j] - 2.0 * U[i][j] + U[i-1][j]) * invRE * invDx*invDx;
			rhsu += (U[i][j+1] - 2.0 * U[i][j] + U[i][j-1]) * invRE * invDy*invDy;
			rhsv += (V[i+1][j] - 2.0 * V[i][j] + V[i-1][j]) * invRE * invDx*invDx;
			rhsv += (V[i][j+1] - 2.0 * V[i][j] + V[i][j-1]) * invRE * invDy*invDy;

			rhsu +=  -U[i][j] * (-U[i+2][j] + 8.0 * (U[i+1][j] - U[i-1][j]) + U[i-2][j]) * inv12 * invDx;
			rhsu += -abs(U[i][j]) * (U[i+2][j] - 4.0 * U[i+1][j] + 6.0 * U[i][j] - 4.0 * U[i-1][j] + U[i-2][j]) * 0.25 * invDx;
			rhsv +=  -U[i][j] * (-V[i+2][j] + 8.0 * (V[i+1][j] - V[i-1][j]) + V[i-2][j]) * inv12 * invDx;
			rhsv += -abs(U[i][j]) * (V[i+2][j] - 4.0 * V[i+1][j] + 6.0 * V[i][j] - 4.0 * V[i-1][j] + V[i-2][j]) * 0.25 * invDx;

			rhsu += -V[i][j] * (-U[i][j+2] + 8.0 * (U[i][j+1] - U[i][j-1]) + U[i][j-2]) * inv12 * invDy;
			rhsu += -abs(U[i][j]) * (U[i][j+2] - 4.0 * U[i][j+1] + 6.0 * U[i][j] - 4.0 * U[i][j-1] + U[i][j-2]) * 0.25 * invDy;
			rhsv += -V[i][j] * (-V[i][j+2] + 8.0 * (V[i][j+1] - V[i][j-1]) + V[i][j-2]) * inv12 * invDy;
			rhsv += -abs(U[i][j]) * (V[i][j+2] - 4.0 * V[i][j+1] + 6.0 * V[i][j] - 4.0 * V[i][j-1] + V[i][j-2]) * 0.25 * invDy;
